...
def predict(image):
    ...
    cuda.memcpy_htod_async(d_input, image, steam)
    # execute model
...
def callback(data):
    ...
    masked_image = image.cpu().numpy()[0]# 3 x 480 x 640
    masked_image[mask > 0.5] = 1 # mask should be 1 x 480 x 640

    global image_count
    rospy.loginfo(f"Inference on image {image_count} in {duration} seconds")
    cv2.imwrite(f"segmented{image_count:05d}.jpg", masked_image)
    image_count += 1
    pub.publish(img_msg)
...
if __name__ == '__main__':
    ...
    #input_batch = np.rand([1, 3, 480, 640], dtype = np.float32)
